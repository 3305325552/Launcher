...
using settings_ui::drawHotkeyInput;
using settings_ui::helpMarker;
using settings_ui::rowCheckbox;
using settings_ui::section;
...
{
    return settings_ui::currentControlPalette();
}
...
{
    return settings_ui::beginCombo(palette, id, preview);
}

void endSettingsCombo(bool open)
{
    settings_ui::endCombo(open);
}

bool settingsComboItem(const UiPalette& palette, const char* label, bool selected)
{
    return settings_ui::comboItem(palette, label, selected);
}
...
void drawSettingsPanel(AppContext& context)
{
    ...
    if (ImGui::Begin("Settings", &settingsOpen,
        ...
        if (!gSettingsSearch.empty()) {
            ...
            case 1: changed |= drawActionSettingsPage(context); break;
            case 2: changed |= drawInterfaceSettingsPage(context); break;
            case 3: changed |= drawSearchSettingsPage(context); break;
